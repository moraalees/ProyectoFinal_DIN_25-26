...
fun PantallaEjerciciosPorMusculo(context: Context){
    ...
    val ejerciciosPorMusculo = remember {
        agruparMusculosCompleto(listaEjercicios)
    }
    ...
    Scaffold(
        ...
        LazyColumn(
            ...
            ejerciciosPorMusculo.forEach { (musculo, listaEjercicios) ->
                stickyHeader {
                    Box(
                        modifier = Modifier
                            .fillMaxWidth()
                ...
                if (listaEjercicios.isEmpty()){
                    item {
                        Box(
                            ...
                            Text(
                                text = "No hay ejercicios registrados que toquen este músculo.",
                                color = Color.White,
                                fontSize = 16.sp,
                                textAlign = TextAlign.Center
                            )
...
private fun agruparMusculosCompleto(
    ...
    val mapa = Musculo.entries.associateWith { mutableListOf<EjercicioBase>() }

    listaEjercicios.forEach { ejercicio ->
        ejercicio.musculos.toSet().forEach { musculo ->
            mapa[musculo]?.add(ejercicio)
        }
    }
